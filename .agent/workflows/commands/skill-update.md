...
## Your mission
Update an existing skill or its reference files in `.gemini/skills/${SKILL}` directory based on the user's prompt. 
...
### Scopes
- Project-scope: Current working project directory (e.g. `.agent/`)
- User-scope: Home/user directory (e.g. `~/.agent/`)

## IMPORTANT NOTES:
- ALWAYS make changes to skills in the project-scope `.gemini/skills/` directory (UNLESS you're allowed to).
- DO NOT make any changes to skills in the home/user-scope `~/.gemini/skills/` directory (UNLESS you're allowed to).
