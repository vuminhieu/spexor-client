...
```bash
node .agent/scripts/worktree.cjs info --json
```
...
**Monorepo:**
```bash
node .agent/scripts/worktree.cjs create "<PROJECT>" "<SLUG>" --prefix <TYPE>
```

**Standalone:**
```bash
node .agent/scripts/worktree.cjs create "<SLUG>" --prefix <TYPE>
```
...
Claude: [Runs: node .agent/scripts/worktree.cjs info --json]
        [Detects: standalone repo, envFiles: [".env.example"]]
        [Detects prefix from "fix" keyword: fix]
        [Converts slug: "login-validation-bug"]
        [Runs: node .agent/scripts/worktree.cjs create "login-validation-bug" --prefix fix]
